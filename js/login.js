...
onAuthStateChanged(auth, async user => {
  ...
  try {
    ...
    else showError("Your Firebase account exists, but no Heritage user profile has been created yet. Ask Head Office to finish your setup.");
  } catch(e) {
    console.error(e);
...
form.addEventListener("submit", async e => {
  ...
  try {
    ...
    loginButton.disabled=false; loginButton.textContent="Sign in";
  }
});
...
document.getElementById("forgotPassword").addEventListener("click",async()=>{
  ...
  try {
    ...
    errorBox.className="alert success"; errorBox.textContent="Password reset email sent. Check your inbox.";
  } catch(e){ showError("We could not send a reset email. Check the email address and Firebase setup."); }
});
